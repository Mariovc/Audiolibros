...
public class ZoomSeekBar extends View {
    ...
    public ZoomSeekBar(Context context, AttributeSet attrs) {
        ...
        try {
            ...
            val = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_val, 160);
            valMin = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_valMin, 100);
            valMax = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_valMax, 200);
            escalaMin = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_escalaMin, 150);
            escalaMax = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_escalaMax, 180);
            escalaIni = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_escalaIni, 100);
            escalaRaya = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_escalaRaya, 2);
            escalaRayaLarga = a.getDimensionPixelSize(
                    R.styleable.ZoomSeekBar_escalaRayaLarga, 5);
        } finally {
            a.recycle();
        }
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        int altoDeseado = altoNumeros + altoRegla + altoBar
                + getPaddingBottom()
                + getPaddingTop();
        yIni = getHeight() / 2 - altoDeseado / 2;
